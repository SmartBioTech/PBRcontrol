...
def test_all():
    ...
    while id < 23:
        ...
        requests.post('https://localhost:5000/1/PBR01', str({'time': t, 'cmd_id': id, 'args': str(pbr[id]), 'source' : 'external'}), verify=False, auth=('BioArInEO', 'sybila'))
        id+=1
        sleep(1)

    while id < 32:
        t = datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S")
        requests.post('https://localhost:5000/1/GAS01', str({'time': t, 'cmd_id': id, 'args': str(gas[id]), 'source' : 'external'}), verify=False, auth=('BioArInEO', 'sybila'))
        sleep(1)
        id+=1

    while id < 35:
        t = datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S")
        requests.post('https://localhost:5000/2/GMS02', str({'time': t, 'cmd_id': id, 'args': str(gms[id]), 'source' : 'external'}), verify=False, auth=('BioArInEO', 'sybila'))
        sleep(1)
        id+=1
...
def add_node(node_number):
    ...
    :return:
    '''
    str_dict = str({node_number: {'experiment_details': {'sleep_time': 10}, 'devices': [{'device_type': 'PBR', 'device_id': 'PBR01', 'device_class': 'PSI_test', 'address': None, 'setup': {'initial_commands': [{'id': 16, 'args': '[1]'}, {'id': 13, 'args': '[50, True]'}, {'id': 8, 'args': '[5, False]'}], 'lower_outlier_tol': 2, 'upper_outlier_tol': 3, 'max_outliers': 6, 'min_OD': 0.1, 'max_OD': 0.9, 'pump_id': 5}}, {'device_type': 'GAS', 'device_id': 'GAS01', 'device_class': 'PSI_test', 'address': None, 'setup': {'initial_commands': []}}]}})
    requests.post("https://localhost:5000/", str_dict, verify=False, auth=('BioArInEO', 'sybila'))

def post_cmd(node, device_id, cmd_id, args):
    '''
    ...
    '''
    t = datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    requests.post('https://localhost:5000/' + str(node) +'/'+ str(device_id),
